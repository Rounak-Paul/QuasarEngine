...
QS_API Qs_Light *qs_light_create(Qs_Engine *engine, const Qs_LightDesc *desc);

/// Destroys a light.
QS_API void qs_light_destroy(Qs_Light *light);

/// Returns the debug name.
QS_API const char *qs_light_name(const Qs_Light *light);

/// Returns a mutable pointer to the lightâ€™s position.
QS_API float *qs_light_position(Qs_Light *light);

/// Returns a mutable pointer to the light’s direction.
QS_API float *qs_light_direction(Qs_Light *light);

/// Returns a mutable pointer to the light’s color.
QS_API float *qs_light_color(Qs_Light *light);

QS_API void  qs_light_set_intensity(Qs_Light *light, float intensity);
QS_API float qs_light_intensity(const Qs_Light *light);
QS_API void  qs_light_set_range(Qs_Light *light, float range);
QS_API void  qs_light_set_cone(Qs_Light *light, float inner_deg, float outer_deg);
QS_API void  qs_light_set_enabled(Qs_Light *light, bool enabled);
QS_API bool  qs_light_enabled(const Qs_Light *light);

/// Returns true if the light is active and should be submitted for rendering.
QS_API bool  qs_light_is_active(const Qs_Light *light);

/// Packs a light's data into a GPU-ready struct for UBO upload.
QS_API void  qs_light_pack_gpu(const Qs_Light *light, Qs_LightGPU *out);
