...
void VulkanCommandBuffer::begin(b8 is_single_use, b8 is_renderpass_continue, b8 is_simultaneous_use)
{
    ...
    VK_CALL(vkBeginCommandBuffer(_handle, &begin_info));
    _state = COMMAND_BUFFER_STATE_RECORDING;
}
void VulkanCommandBuffer::end()
{
    VK_CALL(vkEndCommandBuffer(_handle));
    _state = COMMAND_BUFFER_STATE_RECORDING_ENDED;
}
void VulkanCommandBuffer::update_submitted()
{
    _state = COMMAND_BUFFER_STATE_SUBMITTED;
}
void VulkanCommandBuffer::reset()
{
    // VK_CALL(vkResetCommandBuffer(_handle, 0));
    _state = COMMAND_BUFFER_STATE_READY;
}
