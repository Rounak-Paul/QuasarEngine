...
void _destroy(vulkan_context* context, vulkan_swapchain* swapchain);
...
b8 _create(vulkan_context* context, VkExtent2D swapchain_extent, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)QSMEM.allocate(sizeof(VkImage) * swapchain->image_count);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)QSMEM.allocate(sizeof(VkImageView) * swapchain->image_count);
    }
