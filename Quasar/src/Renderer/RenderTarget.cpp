...
namespace Quasar
{
    b8 RenderTarget::create()
    {
        ...
        commandBuffer.allocate_and_begin_single_use(context, context->command_pool);
        ...
        commandBuffer.end_single_use(context, context->command_pool, context->device.graphics_queue);
        ...
        for (u8 i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkCreateFramebuffer(context->device.logical_device, &framebufferInfo, nullptr, &_framebuffer[i]);
        }
    ...
    b8 RenderTarget::render(Math::Extent extent, const VkClearColorValue &bg_color, u8 frame_index)
    {
        ...
        VulkanCommandBuffer *commandBuffer = &context->command_buffers[frame_index];
        ...
        vkCmdBeginRenderPass(commandBuffer->_handle, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            VulkanShader *shader = (VulkanShader*)_shader._internal_data;
            VulkanPipeline *pipeline = shader->pipelines[0];
            vkCmdBindPipeline(commandBuffer->_handle, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->handle);
            ...
            vkCmdBindDescriptorSets(commandBuffer->_handle, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->pipeline_layout, 0, 1, &pipeline->descriptor_sets[frame_index], 0, nullptr);
    ...
    void RenderTarget::updateUniformBuffer(u32 index)
    {
        ...
        VulkanShader *shader = (VulkanShader*)_shader._internal_data;
        VulkanPipeline *pipeline = shader->pipelines[0];
        memcpy(pipeline->uniform_buffers_mapped[index], &ubo, sizeof(ubo));
    }
}