...
namespace Quasar
{
VkDescriptorSet main_scene_descriptor_set;
...
void Scenespace::update(render_packet* packet) {
    ...
    if (scene->update(content_region.x, content_region.y, ImVec4ToClearColor(ImGui::GetStyleColorVec4(ImGuiCol_WindowBg)))) {
        ...
        main_scene_descriptor_set = ImGui_ImplVulkan_AddTexture(QS_RENDERER.get_vkcontext()->_texture_sampler, scene->_render_target.get_resolve_image_view(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
